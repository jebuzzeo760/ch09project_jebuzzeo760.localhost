...
namespace EKLIB\StellarWP\DB;

use Closure;
use Exception;
use Generator;
use EKLIB\StellarWP\DB\Database\Exceptions\DatabaseQueryException;
use EKLIB\StellarWP\DB\QueryBuilder\Clauses\RawSQL;
use EKLIB\StellarWP\DB\QueryBuilder\QueryBuilder;
use WP_Error;
...
class DB {
	...
	public static function __callStatic( $name, $arguments ) {
		return self::runQueryWithErrorChecking(
			static function () use ( $name, $arguments ) {
				...
				if ( in_array( $name, array( 'get_row', 'get_col', 'get_results', 'query' ), true ) ) {
	...
	public static function generate_results( ?string $query = null, string $output = OBJECT, int $batch_size = 20 ): Generator {
		yield from self::run_batched_query(
			$query,
			$batch_size,
			static function ( string $run_query ) use ( $output ) {
	...
	public function generate_col( ?string $query = null, int $x = 0, int $batch_size = 50 ): Generator {
		yield from self::run_batched_query(
			$query,
			$batch_size,
			static function ( string $run_query ) use ( $x ) {
